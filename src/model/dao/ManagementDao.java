...
public class ManagementDao extends IBaseDao{
    ...
    public List<ManagementDto> mfind(){
        ...
        try {
            String sql = "SELECT mno, mdate, pno, mstatus, in_date, out_date FROM management mm JOIN product pd ON mm.pno = pd.pno";
            PreparedStatement ps = conn.prepareStatement(sql);
            ...
            while (rs.next()) { // rs.next() :다음 레코드로 이동, 마지막 레코드까지 반복
                ...
                md.setOut_date(rs.getDate("out_date").toLocalDate());
